// tests/e2e-checklists/helpers/env.ts — TASK-008/TEST-009 (P12.3, 08_Fases_y_Backlog.md F12)
//
...
// `tests/e2e-clients-sites/`, `tests/e2e-shifts-services/`, `tests/e2e-assignments/`, etc.):
// VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY, SUPABASE_SERVICE_ROLE_KEY, SEED_DEV_PASSWORD. Se leen
// de `app/.env.local`; nunca se imprime ningún valor. Copia deliberada de esas suites (no un
// import cruzado): cada suite de backend real queda autocontenida.

export interface E2eChecklistsEnv {
  supabaseUrl: string
  anonKey: string
...
let cached: E2eChecklistsEnv | null | undefined

/** `null` si falta alguna variable (para que los specs se salteen solos con `test.skip`). */
export function readE2eChecklistsEnv(): E2eChecklistsEnv | null {
...
export const MISSING_ENV_MESSAGE =
  '[tests/e2e-checklists] Faltan VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY, ' +
  'SUPABASE_SERVICE_ROLE_KEY o SEED_DEV_PASSWORD en .env.local. Ver tests/e2e-checklists/README.md ' +
