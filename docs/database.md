...
son F14, `0027_rpc_notices_admin_attendance.sql` -- ver la sección siguiente.
...
## RPC de avisos y asistencia administrativa (04 sección 6.2, 9, 06 sección 10 y 11, migración `0027`, ABS-002, ATT-007, F14 · Ausencias y demoras, asistencia administrativa, P14.1)

`notify_delay`, `notify_absence`, `admin_record_attendance` y `close_assignment` -- las cuatro RPC
que faltaban de `04` sección 9 y `06` sección 10 y 11. Decisiones ratificadas por Mike el 26 sep
2026 (P14.0, `02_Decisiones.md`): P-072 (minutos obligatorios en la demora), hora editable en
`admin_record_attendance`/`close_assignment` (`p_at` opcional, entre las 0:00 del día del turno,
hora de Argentina, y `now()`, nunca futura), y ausencia en nombre del empleado aceptada después del
inicio efectivo mientras la asignación no tenga inicio registrado.

| RPC                                                                 | Quién                                                                | Qué hace                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------------------------------------------------------- | -------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `notify_delay(p_assignment_id, p_minutes, p_reason_text?)`          | E (propia); O, A + `manage_attendance` (en nombre)                   | Aviso de demora (P-072, P-074). Solo antes del inicio efectivo, para todos los llamadores -- `TOO_LATE_TO_NOTIFY` si no (sin excepción para el aviso en nombre, ratificado el 26 sep 2026). `p_minutes` obligatorio, 1..600 (`MINUTES_REQUIRED`, mismo tope que el `check` de la tabla, `0009`). `ASSIGNMENT_NOT_FOUND`, `NOT_YOUR_ASSIGNMENT` (empleado sobre asignación ajena), `FORBIDDEN` (supervisor, o admin sin `manage_attendance`), `SHIFT_CANCELLED`/`SHIFT_COMPLETED`, `ASSIGNMENT_STARTED` (ya `present`/`finished`). Inserta `attendance_notices` (`kind = delay`, `reported_by = auth.uid()`, `source` según quién avisa); asignación → `delay_notified`. Devuelve la fila de `attendance_notices`.                                                                                                                                                                                                                                              |
| `notify_absence(p_assignment_id, p_reason_code, p_reason_text?)`    | E (propia, antes del inicio); O, A + `manage_attendance` (en nombre) | Aviso de ausencia (P-073, P-074). El propio empleado, solo antes del inicio efectivo. Dueño y administrador con `manage_attendance` también después del inicio efectivo, mientras la asignación no tenga inicio registrado (`status in (expected, delay_notified)`, que por construcción implica que todavía no hay `check_in` -- decisión de Mike, 26 sep 2026, P14.0). `REASON_REQUIRED` sin motivo, o con `other` sin texto. Mismos `ASSIGNMENT_NOT_FOUND`/`NOT_YOUR_ASSIGNMENT`/`FORBIDDEN`/`SHIFT_CANCELLED`/`SHIFT_COMPLETED`/`ASSIGNMENT_STARTED` que `notify_delay`. Asignación → `absence_notified`; el cupo **no** se libera solo (P-073). Devuelve la fila de `attendance_notices`.                                                                                                                                                                                                                                                                 |
| `admin_record_attendance(p_assignment_id, p_kind, p_at?, p_reason)` | O; A + `manage_attendance`                                           | Registra inicio o fin en nombre del empleado (P-075). `p_at` opcional (por defecto `now()`), entre las 0:00 del día del turno (Argentina, `app.local_ts(shift_date, '00:00')`) y `now()`, nunca futura -- `AT_OUT_OF_RANGE` (código nuevo) si no. Motivo siempre obligatorio (`REASON_REQUIRED`). `p_kind = check_in`: mismos códigos y transición que `record_check_in` (`ASSIGNMENT_NOT_FOUND`, `SHIFT_CANCELLED`/`SHIFT_COMPLETED`, `ALREADY_CHECKED_IN`; asignación → `present`, turno → `in_progress` si corresponde). `p_kind = check_out`: mismos códigos que `record_check_out` (`NOT_CHECKED_IN`, `ALREADY_CHECKED_OUT`) más `INVALID_TIME_RANGE` si `p_at` no es posterior al `check_in` ya registrado (la única RPC de asistencia donde el fin no lo pone `now()` en el instante exacto de la llamada); asignación → `finished`, turno → `completed` si corresponde. Inserta `attendance_records` con `source = admin`, `recorded_by = auth.uid()`. |
| `close_assignment(p_assignment_id, p_reason, p_at?)`                | O; A + `manage_attendance`                                           | Cierre manual de una asignación `present` sin fin (P-069). Atajo semántico de `admin_record_attendance(p_kind := check_out)`: mismos códigos (`NOT_CHECKED_IN`, `ALREADY_CHECKED_OUT`, `INVALID_TIME_RANGE`, `AT_OUT_OF_RANGE`), motivo siempre obligatorio. `p_at` agregado sobre la firma original de `06` sección 10 (decisión menor del encargo P14.1, ver más abajo). Inserta `check_out` con `source = admin`; turno → `completed` si corresponde.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |

Decisiones menores (documentadas también en el reporte de la tarea):

- Permisos de `notify_delay`/`notify_absence`: árbol de decisión -- si el llamador es owner o
  admin, manda `manage_attendance` (`FORBIDDEN` si falta); si no, tiene que ser employee y dueño de
  la asignación (`NOT_YOUR_ASSIGNMENT` si no); cualquier otro caso (supervisor puro) → `FORBIDDEN`.
  A diferencia de `set_assignment_notes` (0026, que no distingue "asignación ajena" de "sin
  permiso"), acá sí se distingue porque `06` sección 11 lista `NOT_YOUR_ASSIGNMENT` como código
  propio de esta sección.
- `ASSIGNMENT_STARTED` (código ya existente, `06` sección 8) se reutiliza en `notify_delay`/
  `notify_absence` cuando la asignación ya tiene inicio registrado (`present`/`finished`): ninguna
  de las dos RPC tiene sentido de negocio a esa altura.
- Repetir un aviso del mismo tipo, o avisar demora estando ya en `absence_notified`: se reutiliza
  `TOO_LATE_TO_NOTIFY` como código general (no hay uno dedicado en `06` sección 15 para esa
  situación puntual, y el mensaje sigue siendo razonable).
- `AT_OUT_OF_RANGE` (código nuevo): no está en `06` sección 15 -- la hora editable de
  `admin_record_attendance`/`close_assignment` es una decisión ratificada recién el 26 sep 2026
  (P14.0), posterior a esa lista.
- `admin_record_attendance` no exige `NOT_TODAY`: `06` sección 10 no lo pide para esta RPC (a
  diferencia de `record_check_in`), y la validación de rango de `p_at` ya impide cargar asistencia
  de un turno cuyo día todavía no llegó; sí permite cargar la de un turno de un día anterior
  (P-069, "sin cierre automático a fin de día"), igual que `record_check_out`.
- `close_assignment` con `p_at` (no estaba en la firma de `06` sección 10, que solo traía
  `p_assignment_id, p_reason`): el encargo P14.1 lo pidió explícitamente ("Hora editable en
  `admin_record_attendance` (y `close_assignment` si recibe `p_at`)"); queda documentado acá como
  la corrección a `06_API.md` (actualizado en el mismo paquete).
- `app.start_shift_if_needed(p_shift_id)`/`app.complete_shift_if_done(p_shift_id)` (auxiliares
  nuevos en `app`): se extrajo la lógica de transición de turno que hasta `0026` vivía inline en
  `record_check_in`/`record_check_out`, y esta migración reemplaza (`create or replace`) el cuerpo
  de esas dos funciones para que llamen a los auxiliares en vez de repetirla -- mismo
  comportamiento (verificado con los pgTAP ya existentes de `0026`, que siguieron pasando), y
  `admin_record_attendance`/`close_assignment` la reutilizan en vez de duplicarla.
- `v_assignments_board`/`v_my_day` (`create or replace view`, columnas nuevas al final): último
  aviso y origen del registro -- ver la tabla de "Vistas" más arriba.

Detalle completo en `supabase/tests/0027_rpc_notices_admin_attendance.test.sql` (67 aserciones: las
cuatro RPC, todos sus códigos de error, permisos por rol -- owner, admin con y sin
...
checklists" más arriba. En F13 (P13.1, ATT-001, ATT-002): `0026_rpc_attendance.sql` --
`record_check_in`, `record_check_out` y `set_assignment_notes` -- ver "RPC de asistencia del
